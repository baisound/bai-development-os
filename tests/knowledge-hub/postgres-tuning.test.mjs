...
test('8 GiB scale-up PostgreSQL profile adds headroom without linear per-query memory growth',()=>{
 const text=read('deploy/knowledge-hub/postgres/postgresql.tuned-8gb.conf');
 assert.equal(value(text,'shared_buffers'),'1536MB');assert.equal(value(text,'effective_cache_size'),'5GB');assert.equal(value(text,'work_mem'),'8MB');assert.equal(value(text,'max_connections'),'64');
 assert.equal(value(text,'maintenance_work_mem'),'256MB');assert.equal(value(text,'autovacuum_work_mem'),'256MB');
 assert.equal(value(text,'fsync'),'on');assert.equal(value(text,'synchronous_commit'),'on');assert.equal(value(text,'full_page_writes'),'on');
 assert.doesNotMatch(text,/^random_page_cost\s*=|^effective_io_concurrency\s*=/m);
});

test('Compose mounts tuning profile and initializes checksums plus SCRAM without exposing PostgreSQL',()=>{
 const compose=read('deploy/knowledge-hub/compose.yaml');assert.match(compose,/postgres:16\.14-alpine/);assert.match(compose,/POSTGRES_HOST_AUTH_METHOD: "scram-sha-256"/);assert.match(compose,/POSTGRES_INITDB_ARGS: "--data-checksums"/);assert.match(compose,/POSTGRES_CONFIG_FILE:-\.\/postgres\/postgresql\.tuned-4gb\.conf/);assert.match(compose,/POSTGRES_SHM_SIZE:-512mb/);assert.match(compose,/config_file=\/etc\/postgresql\/postgresql\.conf/);assert.doesNotMatch(compose,/5432:5432/);
});
...
test('environment bootstrap and active tuning verification scripts are syntax-safe and secret-free in repo',()=>{
 ...
 const example=read('deploy/knowledge-hub/.env.example');assert.match(example,/POSTGRES_PASSWORD=REPLACE_AT_DEPLOYMENT/);assert.match(example,/POSTGRES_CONFIG_FILE=\.\/postgres\/postgresql\.tuned-4gb\.conf/);assert.match(example,/POSTGRES_SHM_SIZE=512mb/);assert.doesNotMatch(example,/POSTGRES_PASSWORD=[0-9a-f]{32,}/);
});
...
test('environment bootstrap generates 0600 host-only env without printing the DB password',()=>{
 ...
 const text=fs.readFileSync(out,'utf8');assert.match(text,/POSTGRES_CONFIG_FILE=\.\/postgres\/postgresql\.tuned-4gb\.conf/);assert.match(text,/POSTGRES_SHM_SIZE=512mb/);const password=text.match(/^POSTGRES_PASSWORD=([0-9a-f]+)$/m)?.[1];assert.ok(password&&password.length===64);assert.ok(!r.stdout.includes(password));
