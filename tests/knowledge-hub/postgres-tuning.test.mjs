...
test('environment bootstrap and active tuning verification scripts are syntax-safe and secret-free in repo',()=>{
 ...
 const example=read('deploy/knowledge-hub/.env.example');assert.match(example,/POSTGRES_PASSWORD=REPLACE_AT_DEPLOYMENT/);assert.match(example,/POSTGRES_CONFIG_FILE=\.\/postgres\/postgresql\.tuned-8gb\.conf/);assert.match(example,/POSTGRES_SHM_SIZE=1gb/);assert.doesNotMatch(example,/POSTGRES_PASSWORD=[0-9a-f]{32,}/);
});
...
test('environment bootstrap generates 0600 host-only env without printing the DB password',()=>{
 ...
 const text=fs.readFileSync(out,'utf8');assert.match(text,/POSTGRES_CONFIG_FILE=\.\/postgres\/postgresql\.tuned-8gb\.conf/);assert.match(text,/POSTGRES_SHM_SIZE=1gb/);const password=text.match(/^POSTGRES_PASSWORD=([0-9a-f]+)$/m)?.[1];assert.ok(password&&password.length===64);assert.ok(!r.stdout.includes(password));
