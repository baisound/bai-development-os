...
if(/POSTGRES_CONFIG_FILE:-|POSTGRES_SHM_SIZE:-/.test(compose))failures.push('compose: host-memory profile must not silently fall back');
if(/5432:5432/.test(compose))failures.push('compose: PostgreSQL must not be host-published');
if(failures.length){console.error(JSON.stringify({status:'FAIL',failures},null,2));process.exit(1);}
console.log(JSON.stringify({status:'PASS',profiles:['2gb','4gb','8gb'],profile_selection:'EXPLICIT_REQUIRED',durability:'PRESERVED',public_activation:'NOT_AUTHORIZED'},null,2));
