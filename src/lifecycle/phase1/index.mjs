...
export function validateTransition(current, request) {
  ...
  if (TERMINAL.has(current.task_status) && to.task_status !== current.task_status) throw new LifecycleError('TERMINAL_REOPEN_FORBIDDEN', 'terminal task cannot reopen');
  if (to.archive_status !== 'NOT_ELIGIBLE' && to.archive_status !== current.archive_status) throw new LifecycleError('UNDEFINED_TRANSITION', 'archive operation is outside phase 1');
...
export class LifecycleStore {
  ...
  async transition(request, appliedBy) {
    ...
    try {
      ...
      const next = { ...current, ...request.to, record_revision: current.record_revision + 1, status_reason: request.reason, requested_by: request.requested_by, authorized_by: request.authorized_by, applied_by: appliedBy, authorization_reference: request.authorization_reference, authoritative_evidence: request.evidence, updated_at: now, last_verified_at: now, verification_result: 'PASS', last_transition_id: request.request_id };
