...
export async function issueRoleActivationPermit({
  ...
  preflight_result, preflight_result_checksum, override_binding, now = Date.now(),
}) {
  ...
  const permit_binding = override_binding
    ...
    : undefined;
  const permit = { permit_id: randomUUID(), project_id, task_id, role, session_id, selected_input_set_checksum,
...
async function readLedger(session) {
  let text = '';
  try { text = await readFile(ledgerPath(session), 'utf8'); } catch (error) { if (error.code !== 'ENOENT') throw error; }
