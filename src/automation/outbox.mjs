...
import { atomicWrite, deepFreeze, normalizeRel, resolveWritableInside, stable } from './util.mjs';
...
export function signVerifiedOutboxEvent(event,signer){return signEnvelope(structuredClone(event),signer);}
export function validateVerifiedOutboxEvent(event,canonicalRead,security=null){
  ...
  if(security?.require_signed===true){try{verifySignedEnvelope(event,{public_key:security.public_key,expected_key_id:security.expected_key_id??null});}catch(error){throw new DerivedSyncError(error.code==='SECURITY_SIGNATURE_INVALID'?'OUTBOX_SIGNATURE_INVALID':'OUTBOX_SIGNATURE_REQUIRED',error.message);}}
...
export async function consumeDerivedSyncOutbox(root,{event,canonical_read,owner_authorized=false,consumer,security=null,ack_signer=null}={}){
  validateVerifiedOutboxEvent(event,canonical_read,security); if(!owner_authorized) throw new DerivedSyncError('OUTBOX_CONSUMER_NOT_AUTHORIZED'); if(typeof consumer!=='function') throw new DerivedSyncError('OUTBOX_CONSUMER_INVALID');
  return withLock(root,async()=>{const ackRel='.bai-os/automation/derived-sync-acks.jsonl';const ackPath=await resolveWritableInside(root,ackRel,DerivedSyncError,{escapeCode:'OUTBOX_PATH_ESCAPE'});let text='';try{text=await readFile(ackPath,'utf8');}catch(e){if(e.code!=='ENOENT')throw e;}const rows=text.trim()?text.trim().split('\n').map(JSON.parse):[];const existing=rows.find(x=>x.idempotency_key===event.idempotency_key);if(existing)return deepFreeze({result:'DERIVED_SYNC_ALREADY_ACKNOWLEDGED',ack:existing,canonical_completion_rolled_back:false});
    ...
    let ack={ack_version:'1.0.0',task_id:event.task_id,idempotency_key:event.idempotency_key,canonical_checksum:event.canonical_checksum,derived_result,ack_checksum:null};ack.ack_checksum=hash({...ack,ack_checksum:null});if(ack_signer)ack=signEnvelope(ack,ack_signer);await atomicWrite(root,ackRel,Buffer.from(`${text}${text&&!text.endsWith('\n')?'\n':''}${JSON.stringify(ack)}\n`),DerivedSyncError);return deepFreeze({result:'DERIVED_SYNC_ACKNOWLEDGED',ack,canonical_completion_rolled_back:false});});
}
