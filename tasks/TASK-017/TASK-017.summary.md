...
Deployment Readiness, Integration Kit RC, Pre-Live Hardening, Docker Compose/PostgreSQL tuning, and a GitHub Actions real Docker/PostgreSQL live-gate workflow are accepted locally. The Product-owned Python client now supports short-lived presigned Object Storage fallback without acknowledging Outbox Events. The default Compose profile now uses PostgreSQL 16.14 with the Owner-selected 4 GiB startup-production tuning profile. A separate 8 GiB scale-up profile is prepared but remains inactive until the host is actually resized and verified; the 2 GiB profile remains a low-resource development/rehearsal option. The GitHub Actions workflow is ready to remove the local-Docker blocker, but its remote run Evidence is still pending. Production VPS/DNS/TLS/real credential activation and BAI VIDEO PRODUCTION TASK-036 real pilot remain separately gated.
