...
- Priority: `P0_MAXIMUM`
- Status: `ACTIVE / PHASES_B_C_D_E_F_H1_H1_2_PASS / PHASE_G_PARKED_HUMAN_GATE / H2_WAITING_PILOT_EVIDENCE`
- DEV Profile: `DEV_4_FOUNDATION_CRITICAL` (selector score `40`)
- Branch: `autonomy/task-018/safety-first-routing`
...
Phases B-F, Phase H1 Consumer-independent Core Hardening and Phase H1.2 Safety-first Routing are complete. H1 adds Context-to-Failure/Knowledge Candidate bridging, single-worker Session Lease safety and fail-closed crash/partial-state Recovery assessment. H1.2 permits verified Context Cost to break only quality-and-reliability ties among already eligible routes; Authority/Safety/DEV/capability/provider/budget gates remain hard floors. Phase G BAI VIDEO PRODUCTION Pilot is parked while that Product is separately developed locally; its checkout was not inspected or changed. Phase H2 empirical optimization waits on Pilot Evidence. No external Codex Automation invocation has been performed.

## Current checkpoint

- Decision: `CHECKPOINT_PR_ELIGIBLE`
- Linux regression: `1409 / 1409 PASS` on WSL2 Ubuntu ext4
